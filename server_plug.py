...
def set_iptables_rule():
    rule = f"iptables -A OUTPUT -p tcp --tcp-flags RST RST -s {FIXED_SERVER_IP} -j DROP"
    print(f"[设置规则] {rule}")
    os.system(rule)

def remove_iptables_rule():
    rule = f"iptables -D OUTPUT -p tcp --tcp-flags RST RST -s {FIXED_SERVER_IP} -j DROP"
    print(f"[删除规则] {rule}")
    os.system(rule)
...
def extract_server_packets(pcap_file, original_server_ip):
    ...
    for pkt in packets:
        if IP in pkt and TCP in pkt and pkt[IP].src == original_server_ip:
            ip_layer = IP(src=FIXED_SERVER_IP, dst=FIXED_CLIENT_IP)
            tcp_layer = TCP(
                sport=pkt[TCP].sport,
                dport=pkt[TCP].dport,
                seq=pkt[TCP].seq,
...
def main():
    PCAP_FILE = "8.pcap"
    ORIGINAL_SERVER_IP = "110.43.63.252"
