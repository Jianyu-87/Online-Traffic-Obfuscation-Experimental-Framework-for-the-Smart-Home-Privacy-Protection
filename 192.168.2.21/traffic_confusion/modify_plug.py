...
def set_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "192.168.2.5", "-d", "192.168.2.2", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "110.43.63.252", "-d", "192.168.2.120", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
...
def clear_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "192.168.2.5", "-d", "192.168.2.2", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "110.43.63.252", "-d", "192.168.2.120", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
...
def packet_handler(packet):
    ...
    if scapy_pkt.haslayer(TCP):
        ...
        if source_ip == "192.168.2.5":
            # 修改 IP 地址：根据源和目的 IP 进行修改
            scapy_pkt[IP].src = "192.168.2.120"
            scapy_pkt[IP].dst = "110.43.63.252"
            print(f"[+] 修改源 IP 为 {scapy_pkt[IP].src}，目的 IP 为 {scapy_pkt[IP].dst}")

        if source_ip == "110.43.63.252":
            # 修改 IP 地址：根据源和目的 IP 进行修改
            scapy_pkt[IP].src = "192.168.2.2"
            scapy_pkt[IP].dst = "192.168.2.5"
            print(f"[+] 修改源 IP 为 {scapy_pkt[IP].src}，目的 IP 为 {scapy_pkt[IP].dst}")
