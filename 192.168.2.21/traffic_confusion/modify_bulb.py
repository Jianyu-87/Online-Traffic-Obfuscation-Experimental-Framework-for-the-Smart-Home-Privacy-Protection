...
def set_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "192.168.2.5", "-d", "192.168.2.2", "-j", "NFQUEUE", "--queue-num", "2"], check=True)
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "14.215.35.235", "-d", "192.168.2.194", "-j", "NFQUEUE", "--queue-num", "2"], check=True)
...
def clear_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "192.168.2.5", "-d", "192.168.2.2", "-j", "NFQUEUE", "--queue-num", "2"], check=True)
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "14.215.35.235", "-d", "192.168.2.194", "-j", "NFQUEUE", "--queue-num", "2"], check=True)
...
def packet_handler(packet):
    ...
    if scapy_pkt.haslayer(TCP):
        ...
        if source_ip == "192.168.2.5":
            ...
            scapy_pkt[IP].src = "192.168.2.194"
            scapy_pkt[IP].dst = "14.215.35.235"
            print(f"[+] 修改源 IP 为 {scapy_pkt[IP].src}，目的 IP 为 {scapy_pkt[IP].dst}")

        if source_ip == "14.215.35.235":
            # 修改 IP 地址：根据源和目的 IP 进行修改
