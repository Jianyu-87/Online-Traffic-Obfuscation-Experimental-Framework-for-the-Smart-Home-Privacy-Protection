...
def fast_read_pcap(input_file, label):
    return_list = []
    flows = {}
    pkt_id = 0  # 初始序列为0
    ...
    with PcapReader(input_file) as pcap_reader:
        for pkt in pcap_reader:  # 遍历pcap数据
            pkt_id += 1
            if pkt_id % 10000 == 0:
                print(f"已处理数据包数量: {pkt_id}")

            # 提取数据包信息（假设 fast_pkt_info 返回 [unit, data]）
            pktinfo1 = fast_pkt_info(pkt)
            if len(pktinfo1) > 0:
                unit = pktinfo1[0]
                data = pktinfo1[1]
                if unit not in flows:
                    flows[unit] = []
                flows[unit].append(data)

    # 将所有数据包作为一个整体返回（不按时间分割）
    if flows:
        list_values = list(flows.values())
