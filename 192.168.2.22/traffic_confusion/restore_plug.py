...
def set_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "192.168.2.2", "-d", "192.168.2.5", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "192.168.2.120", "-d", "110.43.63.252", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
        print("[*] iptables 规则已设置。")
    except subprocess.CalledProcessError as e:
        print(f"[!] 设置 iptables 规则失败: {e}")

def clear_iptables_rules():
    """仅删除当前脚本设置的 iptables 规则"""
    try:
        print("[*] 正在清除脚本设置的 iptables 规则...")
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "192.168.2.2", "-d", "192.168.2.5", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "192.168.2.120", "-d", "110.43.63.252", "-j", "NFQUEUE", "--queue-num", "3"], check=True)
        print("[*] 指定 iptables 规则已清除。")
    except subprocess.CalledProcessError as e:
...
def packet_handler(packet):
    ...
    if scapy_pkt.haslayer(TCP):
        ...
        if source_ip == "192.168.2.2":
            ...
            scapy_pkt[IP].src = "110.43.63.252"
            scapy_pkt[IP].dst = "192.168.2.120"
