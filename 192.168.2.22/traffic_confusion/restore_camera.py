...
def set_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "192.168.2.2", "-d", "192.168.2.5", "-j", "NFQUEUE", "--queue-num", "1"], check=True)
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "192.168.2.155", "-d", "110.43.87.243", "-j", "NFQUEUE", "--queue-num", "1"], check=True)
...
def clear_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "192.168.2.2", "-d", "192.168.2.5", "-j", "NFQUEUE", "--queue-num", "1"], check=True)
        subprocess.run(["sudo", "iptables", "-D", "FORWARD", "-s", "192.168.2.155", "-d", "110.43.87.243", "-j", "NFQUEUE", "--queue-num", "1"], check=True)
...
def packet_handler(packet):
    ...
    if scapy_pkt.haslayer(TCP):
        ...
        if source_ip == "192.168.2.2":
            ...
            scapy_pkt[IP].src = "110.43.87.243"
            scapy_pkt[IP].dst = "192.168.2.155"
