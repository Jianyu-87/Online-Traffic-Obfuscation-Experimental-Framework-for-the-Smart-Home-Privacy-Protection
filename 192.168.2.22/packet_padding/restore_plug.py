...
def set_iptables_rules():
    ...
    try:
        ...
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-d", "192.168.2.120", "-j", "NFQUEUE", "--queue-num", "1"], check=True)
        subprocess.run(["sudo", "iptables", "-A", "FORWARD", "-s", "192.168.2.120", "-j", "NFQUEUE", "--queue-num", "1"], check=True)
...
def modify_tcp_packet(packet):
    ...
    if scapy_pkt.haslayer(TCP):
        ...
        # 根据源 IP 地址修改负载并存储相应的表格
        if source_ip == "192.168.2.120":
            if scapy_pkt[IP].tos == 0x20:
                scapy_pkt[IP].tos = 0x00
                print("可能发生重传")
                real_B_send.clear()
                real_B_received.clear()
                C_thinks_B_received.clear()
                C_thinks_B_send.clear()
                C_thinks_B_received.append({'seq': -1, 'ack': 0, 'len': 0})

            if scapy_pkt[IP].tos == 0x30:
                scapy_pkt[IP].tos = 0x00
                C_thinks_B_received.clear()

            if not C_thinks_B_received:
                if scapy_pkt.haslayer(Raw):
                    # 获取 TCP 数据包的负载内容
                    raw_payload = scapy_pkt[Raw].load
                    # C认为B收到
                    length = len(scapy_pkt[Raw].load)
                    C_thinks_B_received.append({'seq': scapy_pkt[TCP].seq, 'ack': scapy_pkt[TCP].ack, 'len': length})
                    print(f"[+] 存储到 C认为B收到 表格：seq = {scapy_pkt[TCP].seq}, ack = {scapy_pkt[TCP].ack}, len = {length}")
                ...
                if C_thinks_B_received and not C_thinks_B_received[-1]['seq'] == -1:
                    if not scapy_pkt.haslayer(Raw):
                        C_thinks_B_received.append({'seq': seq, 'ack': ack, 'len': 0})
                    if scapy_pkt.haslayer(Raw):
                        # 获取 TCP 数据包的负载内容
                        raw_payload = scapy_pkt[Raw].load
                        length = len(scapy_pkt[Raw].load)
                        # C_thinks_B_received[-1]['len'] = length
                        C_thinks_B_received.append({'seq': seq, 'ack': ack, 'len': length})
                        print(f"[+] 存储到 C认为B收到 表格：seq = {seq}, ack = {ack}, len = {length}")
            ...
            if C_thinks_B_received and not C_thinks_B_received[-1]['seq'] == -1:
                if not real_B_received:
                    if scapy_pkt.haslayer(Raw):
                        ...
                        # 更新 真实B收到 表格（修改后的 seq、ack 和 len）
                        seq_new = scapy_pkt[TCP].seq
                        ack_new = scapy_pkt[TCP].ack
                        length_new = len(scapy_pkt[Raw].load)
                        real_B_received.append({'seq': seq_new, 'ack': ack_new, 'len': length_new})
                        print(f"[+] 存储到 真实B收到 表格：seq = {seq_new}, ack = {ack_new}, len = {length_new}")
                else:
                    scapy_pkt[TCP].seq = real_B_received[-1]['seq'] + real_B_received[-1]['len']
                    if real_B_send:
                        scapy_pkt[TCP].ack = real_B_send[-1]['seq'] + real_B_send[-1]['len']
                    else:
                        scapy_pkt[TCP].ack = C_thinks_B_received[-1]['ack']
                    if not scapy_pkt.haslayer(Raw):
                        real_B_received.append({'seq': scapy_pkt[TCP].seq, 'ack': scapy_pkt[TCP].ack, 'len': 0})
                    if scapy_pkt.haslayer(Raw):
                        length_new = len(scapy_pkt[Raw].load)
                        #real_B_received[-1]['len'] = length_new
                        real_B_received.append({'seq': scapy_pkt[TCP].seq, 'ack': scapy_pkt[TCP].ack, 'len': length_new})
                        print(f"[+] 存储到 真实B收到 表格：seq = {scapy_pkt[TCP].seq}, ack = {scapy_pkt[TCP].ack}, len = {length_new}")

        else:
            if C_thinks_B_received and not C_thinks_B_received[-1]['seq'] == -1:
                if real_B_send:
                    if scapy_pkt[TCP].seq != real_B_send[-1]['seq'] + real_B_send[-1]['len']:
                        print("可能发生重传")
            ...
            if C_thinks_B_received and not C_thinks_B_received[-1]['seq'] == -1:
                # 存储到 真实B发送 表格（未修改前的数据）
                if not scapy_pkt.haslayer(Raw):
                    real_B_send.append({'seq': seq, 'ack': ack, 'len': 0})
                if scapy_pkt.haslayer(Raw):
                    # 获取 TCP 数据包的负载内容
                    raw_payload = scapy_pkt[Raw].load
                    length = len(scapy_pkt[Raw].load)
                    #real_B_send[-1]['len'] = length
                    real_B_send.append({'seq': seq, 'ack': ack, 'len': length})
                    print(f"[+] 存储到 真实B发送 表格：seq = {seq}, ack = {ack}, len = {length}")
                ...
                if not C_thinks_B_send:
                    scapy_pkt[TCP].seq = real_B_send[-1]['seq']
                else:
                    scapy_pkt[TCP].seq = C_thinks_B_send[-1]['seq'] + C_thinks_B_send[-1]['len']
                scapy_pkt[TCP].ack = C_thinks_B_received[-1]['seq'] + C_thinks_B_received[-1]['len']
                if not scapy_pkt.haslayer(Raw):
                    C_thinks_B_send.append({'seq': scapy_pkt[TCP].seq, 'ack': scapy_pkt[TCP].ack, 'len': 0})
                else:
                    length_new = len(scapy_pkt[Raw].load)
                    #C_thinks_B_send[-1]['len'] = length_new
                    C_thinks_B_send.append({'seq': scapy_pkt[TCP].seq, 'ack': scapy_pkt[TCP].ack, 'len': length_new})
                    print(f"[+] 存储到 C认为B发送 表格：seq = {scapy_pkt[TCP].seq}, "f"ack = {scapy_pkt[TCP].ack}, len = {length_new}")
        ...
        packet.set_payload(bytes(scapy_pkt))  # 将修改后的数据包设置为新的负载
